...
metaphor migration generate Invoice payroll
metaphor migration run
...
(`payroll` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed payroll          # run Rust seeders
metaphor migration generate-seeds payroll  # emit SQL seed files
```

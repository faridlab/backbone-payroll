...
metaphor make entity Vendor --module payroll
...
metaphor schema schema validate payroll

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate payroll --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor payroll
...
> `payroll` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate payroll`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> payroll` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration payroll --database-url …`).
4. Regenerate code: `metaphor schema schema generate payroll --target all --force`.

//! PayrollEntry API Integration Tests
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Tests the PayrollEntry CRUD API endpoints.

use chrono::Utc;
use serde_json::{json, Value};
use uuid::Uuid;

use super::crud_test_base::{CrudTestConfig, GenericCrudTest, TestDataGenerator};
use crate::integration::framework::ApiTest;
use crate::integration::helpers::CommonUtils;

// ============================================================================
// PAYROLLENTRY TEST DATA GENERATOR
// ============================================================================

/// Test data generator for PayrollEntry
pub struct PayrollEntryTestData;

impl TestDataGenerator for PayrollEntryTestData {
    fn generate_create_payload(&self, _utils: &CommonUtils) -> Value {
        let now = Utc::now().to_rfc3339();
        json!({
            "id": Uuid::new_v4().to_string(),
            "company_id": Uuid::new_v4().to_string(),
            "period_year": 1,
            "period_month": 1,
            "posting_date": null,
            "status": "draft",
            "salary_expense_account_id": null,
            "salary_payable_account_id": null,
            "total_gross": 0,
            "total_deductions": 0,
            "total_net": 0,
            "journal_id": null,
            "accounting_post_id": null,
            "metadata": json!({}),
        })
    }

    fn generate_update_payload(&self, id: &str, _utils: &CommonUtils) -> Value {
        let now = Utc::now().to_rfc3339();
        json!({
            "id": id,
            "company_id": Uuid::new_v4().to_string(),
            "period_year": 1,
            "period_month": 1,
            "posting_date": null,
            "status": "draft",
            "salary_expense_account_id": null,
            "salary_payable_account_id": null,
            "total_gross": 0,
            "total_deductions": 0,
            "total_net": 0,
            "journal_id": null,
            "accounting_post_id": null,
            "metadata": json!({}),
        })
    }

    fn generate_invalid_payload(&self) -> Value {
        json!({
            // Missing required fields
        })
    }
}

// ============================================================================
// PAYROLLENTRY API TEST
// ============================================================================

/// PayrollEntry API test suite
pub struct PayrollEntryApiTest {
    inner: GenericCrudTest<PayrollEntryTestData>,
}

impl PayrollEntryApiTest {
    pub fn new() -> Self {
        let mut config = CrudTestConfig::new("/api/v1/payroll_entries", "PayrollEntry");
        config.supports_soft_delete = true;
        Self {
            inner: GenericCrudTest::new(config, PayrollEntryTestData),
        }
    }

    pub async fn run_all(&mut self) -> Vec<crate::integration::framework::TestResult> {
        self.inner.run_all().await
    }
}

impl Default for PayrollEntryApiTest {
    fn default() -> Self {
        Self::new()
    }
}

// ============================================================================
// TESTS
// ============================================================================

#[cfg(test)]
mod tests {
    use super::*;

    #[tokio::test]
    async fn test_payroll_entry_crud() {
        let mut test = PayrollEntryApiTest::new();
        let results = test.run_all().await;

        for result in &results {
            println!("{:?}", result);
        }

        // Check all tests passed
        let failed: Vec<_> = results.iter().filter(|r| !r.success).collect();
        assert!(failed.is_empty(), "Some tests failed: {:?}", failed);
    }
}

// ============================================================================
// CUSTOM TESTS
// ============================================================================

// <<< CUSTOM TESTS START >>>
// Add PayrollEntry specific tests here
// <<< CUSTOM TESTS END >>>

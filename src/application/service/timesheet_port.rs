...
impl ApprovedTimesheetInputs for PoolApprovedTimesheet {
    async fn approved_overtime(
        ...
        // Both reads ride the REQUEST-DEDICATED connection when the caller
        // bound an org scope (the composing service's fence posture): a bare
        // pool fetch lands on an unfenced connection where RLS answers zero
        // rows — the port would silently contribute nothing.
        let approval = backbone_orm::company_scope::fetch_optional_scalar_scoped::<Uuid>(
            &self.pool,
            sqlx::query_scalar(
                r#"SELECT a.id

...
/// Create stateless routes for the Payroll module.
///
...
///
/// ```ignore
/// let module = PayrollModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::PayrollModule) -> Router<()> {
    Router::new()
        .merge(create_payroll_entry_routes(module.payroll_entry_service.clone()))
        .merge(create_salary_slip_routes(module.salary_slip_service.clone()))
...
pub fn get_routes(module: &crate::PayrollModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Payroll module.
///
...
pub fn create_combined_routes(module: &crate::PayrollModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Payroll module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::PayrollModule) -> Router<AppState> {
    Router::new()
